...
def self_edit_seed(passage: Dict) -> int:
    return zlib.crc32(passage["title"].encode("utf-8")) % 100_000
...
def generated_questions(policy: Policy, passages: List[Dict], questions_key: str) -> Dict[str, list]:
    ...
    for index, passage in enumerate(passages):
        ...
        pairs_by_title[passage["title"]] = qa_gen.parse_qa_pairs(completion)
        print(f"[questions {questions_key}] {index + 1}/{len(passages)} "
              f"{len(pairs_by_title[passage['title']])} pairs", flush=True)
    questions_path.parent.mkdir(parents=True, exist_ok=True)
    questions_path.write_text(json.dumps(pairs_by_title, ensure_ascii=False, indent=2),
                              encoding="utf-8")
    return pairs_by_title
...
def gpt_self_edits(passages: List[Dict]) -> Dict[str, List[str]]:
    ...
    for index, passage in enumerate(passages):
        ...
        edits_by_title[passage["title"]] = completions
        print(f"[gpt self-edits] {index + 1}/{len(passages)}", flush=True)
    edits_path.parent.mkdir(parents=True, exist_ok=True)
    edits_path.write_text(json.dumps(edits_by_title, ensure_ascii=False, indent=2),
                          encoding="utf-8")
    return edits_by_title
...
def prepare(condition: str, policy: Policy, base_weights, grader: Grader) -> Callable:
    ...
    if condition == "gpt_se":
        edits_by_title = gpt_self_edits(splits.val_passages())
        return lambda passage: edits_by_title[passage["title"]]
    ...
    if condition == "qa0_sup":
        ...
        pairs_by_title = generated_questions(policy, splits.se_rl_passages(), questions_key)
        reward = lambda passage: question_items(passage["title"], pairs_by_title[passage["title"]])
